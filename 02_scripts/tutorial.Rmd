...
ggplot() +
  geom_point(data=data_wide, mapping=aes(x=Date, y=Calcium)) +
  theme_classic()
...
ggplot(data=data_wide, mapping=aes(x=Date, y=Calcium, color=Station.Name,
                                         group=Station.Name)) +
...
ggplot(data=data_wide, mapping=aes(x=Station.Name, y=Calcium, fill=Station.Name)) +
  geom_boxplot() + 
  theme_classic() #+
  # facet_wrap(~Year)
...
``` {r}
# Load in the Synoptic dataset here 
