...
( meta <- read.csv(file.name, header=FALSE, nrows=10)[,1:2] ) #metadata stops on row 10
( headers <- colnames( read.csv(file.name, skip=12, nrows=1) ) ) #headers are on row 12
data <- read.csv(file.name, skip=13, row.names=NULL, header=FALSE) #data is on row 13+
colnames(data) <- headers #set the headers to be the column names of 'data'
...
nrow(dupes) == nrow(means)
all(dupes$ID == means$ID)
